...
- 2026-06-17: MySQL save failures caused by `max_allowed_packet` limits now log the replay archive size and configured packet limit so oversized replay recovery errors are actionable
- 2026-06-17: Plugin shutdown now waits for in-flight MySQL replay writes to finish before closing the Hikari pool, avoiding `No operations allowed after connection closed` errors when auto-record saves during disable
